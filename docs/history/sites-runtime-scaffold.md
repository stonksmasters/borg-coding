# Historical Sites / hosting scaffold

This document preserves the generic Sites/Vinext hosting notes that originally shipped with the repository scaffold. It is not the canonical BORG architecture or product roadmap. Some optional D1/Drizzle example files referenced below were intentionally removed from the live BORG repository during the September 2026 cleanup because BORG uses its own local SQLite persistence.
