...
export function classifyImplementationFailure(
  input: unknown,
  attempt: number,
  maximum: number,
  ...
  if (fatal) {
    return { disposition: "fatal", category: fatal.category, reason: message, action: fatal.action, message, attempt, maximum };
  }
  if (attempt >= maximum) {
    return {
      disposition: "fatal",
      ...
      reason: message,
      action: "Stop the slice and surface the accumulated evidence after the bounded retry budget is exhausted.",
      message,
      attempt,
      maximum,
    };
  ...
  if (retry) {
    return { disposition: "retry", category: retry.category, reason: message, action: retry.action, message, attempt, maximum };
  }
  if (options.noProgress) {
    return {
      ...
      reason: message,
      action: "Preserve the approved plan, re-run workspace preflight, and retry the current slice with the smallest concrete source mutation.",
      message,
      attempt,
      maximum,
    };
  ...
  return {
    ...
    category: "unknown",
    reason: message,
    action: "Stop rather than repeatedly mutating after an unclassified runtime failure.",
    message,
    attempt,
    maximum,
  };
...
export function compactRecoveryEvidence(
  ...
  return [
    `RECOVERY: ${decision.category}. Continue the SAME approved slice; do not rediscover or re-plan the project.`,
    `Failure: ${decision.message.slice(0, 1200)}`,
    `Runtime action: ${decision.action}`,
    `Workspace preflight: ${preflight.contract.kind}; repaired directories: ${repaired}; Git head: ${preflight.gitHead ?? "unknown"}.`,
    ...(warnings.length ? [`Preflight warnings: ${warnings.join(" | ")}`] : []),
    ...(failures.length ? [`Recent tool failures: ${failures.join(" | ")}`] : []),
    "Use worktree_write for new files, worktree_patch for existing files, and inspect only the target files needed for this recovery.",
  ].join("\n");
}
